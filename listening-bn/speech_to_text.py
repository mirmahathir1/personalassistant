"""Bangla microphone speech-to-text using faster-whisper.
...
BANGLA_INITIAL_PROMPT = (
    "বাংলা ভাষার কথোপকথন হুবহু বাংলা লিপিতে লিখুন। "
    "ইংরেজি বা অন্য কোনো লিপিতে রূপান্তর করবেন না।"
)
...
def model_cache_complete(model_path: str) -> bool:
    path = Path(model_path)
    return all((path / filename).exists() for filename in REQUIRED_MODEL_FILES)
...
def ensure_model_available(
    ...
    try:
        ...
        model_path = str(cached_files[0].parent)
        if model_cache_complete(model_path):
            status_callback("Speech model already downloaded.")
            return model_path
        status_callback("Speech model cache is incomplete. Downloading now...")
    except Exception:
        status_callback("Speech model is not fully cached. Downloading now...")

    downloaded_files: list[Path] = []
    total_files = len(REQUIRED_MODEL_FILES)
    for index, filename in enumerate(REQUIRED_MODEL_FILES, start=1):
        status_callback(
            f"Downloading speech model file {index}/{total_files}: {filename}"
        )
    ...
    model_path = str(downloaded_files[0].parent)
    if not model_cache_complete(model_path):
        missing = [
            filename
            for filename in REQUIRED_MODEL_FILES
            if not (Path(model_path) / filename).exists()
        ]
        raise RuntimeError(
            "Speech model download did not produce required files: "
            + ", ".join(missing)
        )
...
def transcribe(
    ...
    segments, _info = model.transcribe(
        ...
        beam_size=beam_size,
        vad_filter=True,
        initial_prompt=BANGLA_INITIAL_PROMPT,
        condition_on_previous_text=False,
    )
    text = " ".join(segment.text for segment in segments).strip()
    if text and not contains_bengali_script(text):
        status_callback(
            "Rejected transcript because it did not contain Bengali script: "
            f"{text!r}"
        )
        return ""
    return text
